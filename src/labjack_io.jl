function setupLabjackBuffers(Vdac0, Vdac1, BitFIO4, BitFIO5)
    ...
    sendBuff[44+1] = 4 + 1 * 128    # FIO4 & Direction = 1 (Output)

    # FIO5
    sendBuff[45+1] = 13             # IOType is BitDirWrite
    sendBuff[46+1] = 5 + 1 * 128    # FIO5 & Direction = 1 (Output)

    # FIO4
    sendBuff[47+1] = 11             # IOType is BitState Write
    sendBuff[48+1] = 4 + UInt8(BitFIO4) * 128  # FIO4 & Bit

    # FIO5
    sendBuff[49+1] = 11             # IOType is BitState Write
    sendBuff[50+1] = 5 + UInt8(BitFIO5) * 128  # FIO5 & Bit
...
function labjackReadWrite(Vdac0)
    sendIt, recordIt = setupLabjackBuffers(0.0, 0.0, true, false)
    
    if conf["LJ"]["FIO"]["Tick_DAC"] == 1
        setLJTDAC(HANDLE, caliInfoTdac, conf["LJ"]["FIO"]["Tick_DAC"], Vdac0, 0.0)
    elseif conf["LJ"]["FIO"]["Tick_DAC"] == 2
        setLJTDAC(HANDLE, caliInfoTdac, conf["LJ"]["FIO"]["Tick_DAC"], 0.0, Vdac0)
    end
